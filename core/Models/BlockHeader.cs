...
public record BlockHeader
{
    ...
    [MessagePack.Key(5)] public byte[] LocktimeScript { get; set; }
    ...
    /// <returns></returns>
    public static byte[] ToMerkleRoot(in byte[] prevMerkelRoot, in ImmutableArray<Transaction> transactions)
    {
    ...
    public IEnumerable<ValidationResult> Validate()
    {
        ...
        if (!((Version >= ushort.MinValue) & (Version <= ushort.MaxValue)))
            results.Add(new ValidationResult("Incorrect number", new[] { "Ver" }));

        if (PrevBlockHash == null) results.Add(new ValidationResult("Argument is null", new[] { "PrevBlockHash" }));
        ...
        if (MerkleRoot == null) results.Add(new ValidationResult("Argument is null", new[] { "MerkleRoot" }));
        if (MerkleRoot != null && MerkleRoot.Length != 32)
            results.Add(new ValidationResult("Range exception", new[] { "MerkleRoot" }));
        if (Locktime < 0) results.Add(new ValidationResult("Range exception", new[] { "Locktime" }));
        try
        {
