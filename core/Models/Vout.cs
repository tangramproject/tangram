...
public record Vout
{
    ...
    [MessagePack.Key(6)] public byte[] S { get; set; }
    [MessagePack.Key(7)] public CoinType T { get; set; }
    [MessagePack.Key(8)] public byte[] D { get; set; }
    ...
    public IEnumerable<ValidationResult> Validate()
    {
        ...
        if (P != null && P.Length != 33) results.Add(new ValidationResult("Range exception", new[] { "Vout.P" }));
        if (T == CoinType.Coinbase)
        {
        ...
        if (D != null && D.Length != 32) results.Add(new ValidationResult("Range exception", new[] { "Vout.D" }));
        return results;
    }
}