...
using Dawn;
using Serilog;
using TangramXtgm.Consensus.Messages;
using TangramXtgm.Consensus.Models;
using TangramXtgm.Consensus.States;

namespace TangramXtgm.Consensus;
...
public sealed class Blockmania : IDisposable
{
    ...
    public Blockmania(Config cfg, ILogger logger)
    {
        ...
        _entries = Channel.CreateBounded<BlockGraph>(channelOptions);
    ...
    private void DeliverRound(ulong round, Dictionary<ulong, string> hashes)
    {
        ...
        while (true)
        {
            ...
            for (var i = 0; i < Blocks.Count; i++)
            {
                ...
                if (b != null) b.Data = info.Data.Block.Data;

                if (info.Max > round) break;

                Max.Remove(info.Data.Block);
                Statess.Remove(info.Data.Block);
                foreach (var dep in info.Data.Dependencies)
                {
            ...
            if (idx > 0) Blocks = Blocks.GetRange(idx, Blocks.Count - idx);
    ...
    private State FindOrCreateState(Entry e)
    {
        State stat;
        if (e.Prev.Valid())
            stat = !Statess.ContainsKey(e.Prev)
                ? new State(new Dictionary<ulong, List<Timeout>>()).Clone(Round)
                : Statess[e.Prev].Clone(Round);
        else
    ...
    private void Process(Entry entry)
    {
        ...
        if (entry.Dependencies.Any())
        {
            ...
            foreach (var dep in entry.Dependencies) state.Delay[dep.Node] = Util.Diff(round, dep.Round) * 10;
        }
        ...
        if (state.Delay.Count > Quorum2f1)
        {
            ...
            if (xval > timeOutVal) timeOutVal = xval;
        }
        else
        {
            foreach (var val in state.Delay.Select(item => item.Value).Where(val => val > timeOutVal)) timeOutVal = val;
        }
        ...
        if (state.Timeouts.ContainsKey(round))
            ...
            {
                if (state.Data.ContainsKey(new Final(tmout.Node, tmout.Round))) continue;
                uint v = 0;
                var skey = new View(tmout.Node, tmout.Round);
                if (state.Data.ContainsKey(skey)) v = (uint)state.Data[skey];
                if (v > tmout.View) continue;
                var hval = "";
                var skey2 = new Prepared(tmout.Node, tmout.Round, tmout.View);
                if (state.Data.ContainsKey(skey2)) hval = (string)state.Data[skey2];
                if (state.Data == null)
        ...
        foreach (var dep in entry.Dependencies)
        {
            ...
            if (Statess.ContainsKey(dep) && Statess[dep] != null) output = Statess[dep].GetOutPut();
    ...
    {
        // _logger.Here().Debug(
        //     "State: {@State}, Sender: {@Sender}, Receiver: {@Receiver}, Origin: {@Origin}, Message: {@Message}",
        ...
        {
            case PrePrepare m:
                if (v != m.View) return null;
                var pp = new PrePrepared(node, round, m.View);
                if (s.Data.ContainsKey(pp)) return null;
            ...
            case Prepare m:
                ...
                if (b.PrepareCount() != Quorum2f1) return null;
                if (b.HasCommit(receiver)) return null;
            ...
            case Commit m:
                ...
                if (b.CommitCount() != Quorum2f1) return null;
                var nr = new NodeRound(node, round);
                if (s.Final.ContainsKey(nr)) return null;
                if (s.Final == null)
            ...
            case ViewChange m:
                ...
                vcs[m.Sender] = m.Hash;
                if (vcs.Count != Quorum2f1) return null;
                ...
                foreach (var hval in vcs.Select(item => item.Value).Where(hval => hval != ""))
                {
                    if (hash != "" && hval != hash)
                        // _logger.Here().Debug(
                        //     "Got multiple hashes in a view change node.id: {@Node}, round: {Round}, hash={@Hash} hash.alt={@HashAlt}",
            ...
            case NewView m:
                ...
                if (s.Data.ContainsKey(viewKey)) return null;
                s.Data ??= new StateKV();
                s.Data[new View(node, round)] = m.View;
                var tval = origin.Round + s.Timeout + 5;
                if (!s.Timeouts.ContainsKey(tval)) s.Timeouts[tval] = new List<Timeout>();
        ...
        ulong receiver, Block origin, IEnumerable<IMessage> messages)
    {
        ...
        {
            if (processed.ContainsKey(msg) && processed[msg]) continue;
            var resp = ProcessMessage(s, sender, receiver, origin, msg);
            processed[msg] = true;
            if (resp != null) outMessages.Add(resp);
        }
    ...
    public async Task AddAsync(BlockGraph data, CancellationToken cancellationToken)
    {
