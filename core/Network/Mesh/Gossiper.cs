...
public class Gossiper : IDisposable
    {
        ...
        public Gossiper(ushort listenPort, uint service, byte[] serviceName, byte[] serviceVersion, byte[] publicKey, ushort servicePort, GossiperOptions options,
            CancellationToken stoppingToken, ILogger logger)
        {
            _options = options;
        ...
        {
            _logger.LogDebug("Gossip.Mesh starting Gossiper on {GossipEndPoint}", _self.GossipEndPoint);
        ...
        private async Task Bootstrap()
        {
            ...
            {
                _logger.LogDebug("Gossip.Mesh no seeds to bootstrap off");
                return;
            }

            _logger.LogDebug("Gossip.Mesh bootstrapping off seeds");
            ...
            _logger.LogDebug("Gossip.Mesh finished bootstrapping");
        }
        ...
        private async void Listener()
        {
            while (true)
            {
                try
                {
                    ...
                    var request = await _udpClient.ReceiveAsync(_cancellationToken).ConfigureAwait(false);
                        ...
                        foreach (var awaitingAck in _awaitingAcks.ToArray())
                        {
                            if (DateTime.UtcNow > awaitingAck.Value.AddMilliseconds(_options.PruneTimeoutMilliseconds))
                            {
                                    ...
                                    (member.State == MemberState.Dead || member.State == MemberState.Left))
                                {
                                    ...
                                    _logger.LogDebug("Gossip.Mesh pruned member {Member}", member);
        ...
        private void UpdateMembers(IPEndPoint senderGossipEndPoint, DateTime receivedDateTime, Stream stream)
        {
                ...
                (selfClaimedState != MemberState.Alive && selfClaimedGeneration == _self.Generation))
            {
                ...
                _logger.LogDebug(
                    "Gossip.Mesh received a claim about self, state:{State} generation:{SelfGeneration}. Raising generation to {Generation}",
            ...
            while (memberEvent != null)
            {
                ...
                lock (_locker)
                {
                    ...
                    else if (member == null)
                    {
                        ...
                        _logger.LogDebug("Gossip.Mesh member added {Member}", member);
                        PushToMemberListeners(memberEvent);
                    }
        ...
        private void WriteMembers(Stream stream, IPEndPoint destinationGossipEndPoint)
        {
            lock (_locker)
            {
                ...
                stream.WriteService(_self.Service);
        ...
        private void UpdateMemberState(IPEndPoint gossipEndPoint, MemberState memberState)
        {
            lock (_locker)
            {
                ...
                {
                    _logger.LogDebug("Gossip.Mesh {MemberState} member {Member}",
